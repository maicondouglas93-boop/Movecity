...
const ACCESS_COOKIE_BY_ACTOR = Object.freeze({
    user: 'userAccessToken',
    captain: 'captainAccessToken',
    admin: 'adminAccessToken',
});
const REFRESH_COOKIE_BY_ACTOR = Object.freeze({
    user: 'userRefreshToken',
    captain: 'captainRefreshToken',
    admin: 'adminRefreshToken',
});
const REFRESH_COOKIE_PATH_BY_ACTOR = Object.freeze({
    user: '/users',
    captain: '/captains',
    admin: '/api/admin',
});
...
const tokenPolicyMetrics = {
    ...
    refreshConflicts: 0,
    refreshLegacyTransportAccepted: 0,
};
...
module.exports.ACCESS_COOKIE_BY_ACTOR = ACCESS_COOKIE_BY_ACTOR;
module.exports.REFRESH_COOKIE_BY_ACTOR = REFRESH_COOKIE_BY_ACTOR;
...
function refreshBodyMigrationEnabled() {
    const cutoff = new Date(process.env.REFRESH_BODY_MIGRATION_UNTIL || 'invalid');
    return Number.isFinite(cutoff.getTime()) && cutoff.getTime() > Date.now();
}

function isNativeDriverRequest(req, actorType) {
    return actorType === 'captain'
        && String(requestHeader(req, 'x-movecity-client') || '').toLowerCase() === 'android-driver';
}

module.exports.resolveRefreshToken = (req, actorType) => {
    assertActorType(actorType);
    const bodyToken = req?.body?.refreshToken;

    // O app nativo usa EncryptedSharedPreferences/Android Keystore e precisa do token
    // no corpo para o fluxo em background, fora do WebView e sem cookie jar.
    if (isNativeDriverRequest(req, actorType) && bodyToken) return bodyToken;

    const actorCookie = req?.cookies?.[REFRESH_COOKIE_BY_ACTOR[actorType]];
    if (actorCookie) return actorCookie;

    // Cookies antigos permanecem aceitos apenas para girar a sessão ao formato novo.
    const legacyCookie = actorType !== 'admin' ? req?.cookies?.refreshToken : null;
    if (legacyCookie && refreshBodyMigrationEnabled()) {
        recordRefreshEvent('refreshLegacyTransportAccepted', 'AUTH_LEGACY_REFRESH_ACCEPTED', actorType);
        return legacyCookie;
    }

    // Testes de API exercitam o contrato sem browser. Em produção, corpo web só entra
    // na janela explícita de migração e com cabeçalho dedicado; fora dela falha fechado.
    if (process.env.NODE_ENV === 'test' && bodyToken) return bodyToken;
    const isMigration = requestHeader(req, 'x-movecity-refresh-migration') === 'v1';
    if (isMigration && refreshBodyMigrationEnabled() && bodyToken) {
        recordRefreshEvent('refreshLegacyTransportAccepted', 'AUTH_LEGACY_REFRESH_ACCEPTED', actorType);
        return bodyToken;
    }
...
module.exports.shouldExposeRefreshToken = (req, actorType, { newSession = false } = {}) => (
    process.env.NODE_ENV === 'test'
    || (isNativeDriverRequest(req, actorType) && (newSession || Boolean(req?.body?.refreshToken)))
);
...
// cookie HttpOnly/store nativo compartilhado já recebe o único sucessor da vencedora.
...
module.exports.revokeAllForUser = async ({ userId, userType, reason = 'revoked' }) => {
    ...
    const { publishRevocation } = require('./sessionRevocation.service');
    await publishRevocation({ scope: 'account', actorType: userType, actorId: userId, reason });
};

// Encerra somente o access token/JTI usado nesta aba ou dispositivo. O refresh da
// mesma sessão é revogado separadamente pelo controller; outras sessões permanecem.
module.exports.revokeAccessSession = async ({ userId, userType, jti, reason = 'logout' }) => {
    assertActorType(userType);
    if (!userId) return;
    const { publishRevocation } = require('./sessionRevocation.service');
    await publishRevocation({
        scope: jti ? 'session' : 'account',
        actorType: userType,
        actorId: userId,
        jti: jti || null,
        reason,
...
module.exports.refreshCookieOptions = (actorType) => {
    assertActorType(actorType);
    const isProduction = process.env.NODE_ENV === 'production';
    return {
        httpOnly: true,
        ...
        path: REFRESH_COOKIE_PATH_BY_ACTOR[actorType]
    };
};
...
module.exports.clearCookieOptions = (actorType) => {
    const { maxAge, ...rest } = module.exports.refreshCookieOptions(actorType);
    return rest;
};
