...
module.exports.getUserProfile = async (id) => {
    ...
    if (cachedUser) {
        // NodeCache é local à instância. Um bloqueio feito por outro processo precisa
        // prevalecer imediatamente sobre o perfil completo guardado por dez minutos.
        const flags = await userModel.findById(id).select('isBlocked').lean();
        if (!flags) {
            deleteCache(cacheKey);
            return null;
        }
        if (typeof cachedUser.set === 'function') {
            cachedUser.set('isBlocked', flags.isBlocked);
            return cachedUser;
        }
        return Object.assign(
            typeof cachedUser.toObject === 'function' ? cachedUser.toObject() : { ...cachedUser },
