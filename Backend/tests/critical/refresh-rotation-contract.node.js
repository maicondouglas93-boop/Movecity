...
test('graça concorrente não cria nem devolve outro refresh token', () => {
    ...
    assert.match(user, /if\s*\(refreshToken\)[\s\S]*res\.cookie\(['"]refreshToken['"]/);
    assert.match(captain, /if\s*\(refreshToken\)[\s\S]*res\.cookie\(['"]refreshToken['"]/);
    assert.match(adminClient, /if\s*\(data\.refreshToken\)\s*localStorage\.setItem\(['"]adminRefreshToken['"]/);
});
