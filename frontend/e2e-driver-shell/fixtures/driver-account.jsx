...
const publicMode = ['login', 'signup'].includes(mode)
if (!publicMode) saveSession('captain', { token: fakeToken }, { syncNative: false })
window.fixtureAccountFailure = mode !== 'wallet'
...
api.defaults.adapter = async config => {
    ...
    let data
    if (path === '/vehicle-categories') data = [{ name: 'car', displayName: 'Carro' }]
    else if (path === '/captains/register') data = { captain: initial, token: fakeToken }
    else if (path === '/captains/wallet') data = { wallet: { creditBalance: 35 } }
    else if (path === '/captains/transactions') data = { transactions: [] }
    else if (path === '/uploads/document') data = { url: 'https://example.test/new-photo.jpg' }
    ...
    return { config, status: path === '/captains/register' ? 201 : 200, statusText: 'OK', data, headers: {} }
}
function Account() {
    const [captain, setCaptain] = useState(publicMode ? null : initial)
    const entry = { login: '/captain-login', signup: '/captain-signup', wallet: '/captain-wallet', documents: '/captain/documents', help: '/captain-help?category=access', approval: '/approval' }[mode]
    return <QueryClientProvider client={queryClient}><SocketContext.Provider value={{ socket: null }}><CaptainDataContext.Provider value={{ captain, setCaptain }}><ToastProvider><MemoryRouter initialEntries={[entry]}>
    ...
    </MemoryRouter></ToastProvider></CaptainDataContext.Provider></SocketContext.Provider></QueryClientProvider>
}
