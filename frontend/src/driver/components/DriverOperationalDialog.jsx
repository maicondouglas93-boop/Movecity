...
// Exclusivo do motorista: não altera BottomSheets do passageiro ou da viagem.
export default function DriverOperationalDialog({ title, children, onClose, busy = false }) {
    const titleId = useId()
    ...
    return createPortal(
        <div data-driver-dialog className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 pt-[max(1rem,env(safe-area-inset-top))]">
            <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}
                ...
                <div className="flex shrink-0 items-center justify-between gap-2 border-b border-line px-4 py-2">
                    ...
                    <button type="button" onClick={onClose} disabled={busy} aria-label="Recolher atendimento"
